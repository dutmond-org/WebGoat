...
pipeline {	
    ...
    withSonarQubeEnv {
      ...
      bat "set && %scannerHome%\\sonar-scanner.bat -Dsonar.java.binaries=**/target/classes -Dsonar.sources=webwolf/src -Dsonar.projectKey=%BRANCH_NAME%"
			...
			steps {
				...
				script {
				  ...
				  if (env.BRANCH_NAME == 'develop' || env.BRANCH_NAME.startsWith('feature')) {
					  echo "Running a snapshot build from branch ${env.BRANCH_NAME} on repository ${repositoryname}"
